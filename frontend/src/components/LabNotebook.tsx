...
export function LabNotebook() {
    ...
    const [selectedModel, setSelectedModel] = useState<"gemini-3-pro-preview" | "claude-opus-4-5" | "deepseek-chat">("deepseek-chat");
    ...
    const handleSaveCredentials = async () => {
        ...
        try {
            ...
            // If successful, close prompt and retry pending run if any
            setShowCredentialPrompt(false);
            if (pendingRun) {
                startExperiment(pendingRun.mode, pendingRun.config);
                setPendingRun(null);
            }
        } catch (err) {
            setCredentialPromptError("Failed to save credentials.");
        } finally {
